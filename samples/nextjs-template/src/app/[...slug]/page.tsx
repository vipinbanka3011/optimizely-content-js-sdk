...
export default async function Page({ params }: Props) {
  ...
  // setPageContext feeds server components via getPageData() (React.cache, request-scoped).
  // Returns the mapped page data, or null when pageContextConfig.enabled is false.
  const pageData = setPageContext(content[0] as Record<string, unknown>, pageContextConfig);

  // When page context is enabled, wrap with PageDataProvider so client
  // components can access the same data via usePageData().
  if (pageData) {
    return (
